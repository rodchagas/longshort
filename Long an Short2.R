...
server <- function(input, output) {
   ...
   observeEvent(input$runif,{
         ...
         data.stock1 <- getSymbols(as.character(input$Ticker1), src="google", 
                                   from = as.Date(input$dateFrom), to=as.Date(input$dateTo))
         data.stock2 <- getSymbols(as.character(input$Ticker2),src="google", 
                                   from = as.Date(input$dateFrom), to=as.Date(input$dateTo))
